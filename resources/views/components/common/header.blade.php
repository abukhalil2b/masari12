            ...
            <div x-data="header"
                ...
                <div class="dropdown flex-shrink-0" x-data="dropdown" @click.outside="open = false">
                    ...
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                        ...
                        <li>
                            <a href="{{ route('admin.message.index') }}" class="dark:hover:text-white"
                                @click="toggle">
                                <svg class="w-4.5 h-4.5 ltr:mr-2 rtl:ml-2 shrink-0" width="18" height="18"
                                    viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path opacity="0.5"
                        ...
                        @foreach (Auth::user()->profiles as $profile)
                            <li class="border-t border-white-light dark:border-white-light/10">
                                <a href="{{ route('switch-account', $profile->id) }}"
                                    class="pr-4 p-1 w-full bg-[#fffbf6] text-[#bd6e1e] {{ Auth::user()->profile_using == $profile->title ? 'opacity-30' : '' }}">
